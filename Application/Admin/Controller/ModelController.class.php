...
class ModelController extends AdminController {
	...
	public function del() {
		...
		foreach ( $ids as $value ) {
			if($value){
				echo $value;
			$res = D ( 'Model' )->del ( $value );	echo $res;
			if (! $res) {
				break;
			}
			}
...
sql;
			...
			foreach ( $data as $d ) {
				...
				$sql .= "INSERT INTO `{$prefix}{$name}` (" . rtrim ( $field, ',' ) . ') VALUES (' . rtrim ( $value, ',' ) . ");\r\n";
			}
			...
			$sql .= 'INSERT INTO `'.$prefix.'model` (' . rtrim ( $field, ',' ) . ') VALUES (' . rtrim ( $value, ',' ) . ');' . "\r\n";
			...
			foreach ( $list as $k => $vo ) {
				...
				$sql .= 'INSERT INTO `'.$prefix.'attribute` (' . rtrim ( $field, ',' ) . ') VALUES (' . rtrim ( $value, ',' ) . ');' . "\r\n";
			}
			$sql .= 'UPDATE `'.$prefix.'attribute` SET model_id= (SELECT MAX(id) FROM `'.$prefix.'model`) WHERE model_id=0;';
